...
tau_root, weights = np.polynomial.legendre.leggauss(N_collocation - 1)  # Gauss-Legendre points
...
# Number of collocation points (including endpoints)
N_collocation = 5  # Example

# Compute the derivative of the Legendre polynomial of degree N-1
n = N_collocation - 1  # Degree of the Legendre polynomial
P = legendre(n)  # Legendre polynomial
dP = P.deriv()  # Derivative of the Legendre polynomial

# Compute the roots of the derivative (Gauss-Lobatto points, excluding endpoints)
tau_root = np.roots(dP)  # Roots of the derivative

# Include the endpoints -1 and 1
tau_root = np.append(-1, tau_root)  # Include the left endpoint
tau_root = np.append(tau_root, 1)  # Include the right endpoint

# Sort the points
tau_root = np.sort(tau_root)

# Shift from [-1, 1] to [0, 1] (if needed)
tau_root = (tau_root + 1) / 2

print("tau_root_1", tau_root)


# # Compute Gauss-Lobatto collocation points
# # The roots of the derivative of the (N-1)-th Legendre polynomial give the Gauss-Lobatto points (excluding endpoints)
# tau_root = np.polynomial.legendre.leggauss(N_collocation - 2)[0]  # Roots of Legendre polynomial (N-2 points)
# tau_root = np.append(-1, tau_root)  # Include the left endpoint
# tau_root = np.append(tau_root, 1)  # Include the right endpoint
...
# print("tau_root 222",tau_root)
# exit()

# Legendre polynomial basis functions using CasADi
def legendre_basis(tau, tau_root, degree):
    """
    Compute the Legendre polynomial basis functions.
    """
    N = len(tau_root)
    L = []
    for i in range(degree + 1):
        ...
        coeffs = np.zeros(i + 1)
        coeffs[-1] = 1  # Highest degree coefficient
        L.append(np.polynomial.legendre.Legendre(coeffs)(tau))
        print("L",L)
    return L

# Compute the differentiation matrix using Legendre polynomials
def legendre_differentiation_matrix(tau_root):
    """
    Compute the differentiation matrix using Legendre polynomials.
    """
    N = len(tau_root)
    ...
    L = legendre_basis(tau, tau_root, N - 1)
...
# Compute the quadrature weights using Legendre polynomials
def legendre_quadrature_weights(tau_root):
    """
    Compute the quadrature weights using Legendre polynomials.
    """
    N = len(tau_root)
...
# Compute the collocation matrix using Legendre polynomials
def legendre_collocation_matrix(tau_root):
    """
    Compute the collocation matrix using Legendre polynomials.
    """
    N = len(tau_root)
    ...
    for j in range(N):
        for k in range(N):
            # Evaluate Legendre polynomial at collocation points
            C[j, k] = ca.substitute(L[k], tau, tau_root[j])

    return C

# Extend the differentiation matrix to handle the state vector
def extend_differentiation_matrix_legendre(D, state_size):
    """
...
# Example usage
C = legendre_collocation_matrix(tau_root)
D = legendre_differentiation_matrix(tau_root)
B = legendre_quadrature_weights(tau_root)



# Extend matrices for state vector
state_size = 2  # Example state size
C_ext = ca.kron(C, ca.MX.eye(state_size))
D_ext = extend_differentiation_matrix_legendre(D, state_size)
...
N_intervals = N_points-1  # Number of intervals
state_size = 16  # Size of the state vector
control_size = 2  # Size of the control vector

# # Get Gauss-Lobatto collocation points
# tau_root = np.polynomial.legendre.leggauss(N_collocation - 1)  # Roots of Legendre polynomial
# print("tau_root",tau_root)
# tau_root = np.append(-1, tau_root)  # Include the left endpoint for Gauss-Lobatto
# print("tau_root",tau_root)
# tau_root = (tau_root + 1) / 2  # Shift to [0, 1] for Gauss-Lobatto
...
# xi = np.concatenate(([-1], tau_root, [1]))  # Include endpoints

# xi = (xi+1)/2


xi = tau_root
print("Gauss-Lobatto collocation points (shifted to [0, 1]):", tau_root)

# Initialize list to store all tau points
tau_all = []

# Loop over intervals
for k in range(len(teval)-1):
    t_k = teval[k]  # Start of interval
    t_k_plus_1 = teval[k + 1]  # End of interval

    # Map Gauss-Lobatto points to [t_k, t_{k+1}]
    tau_j = t_k + (t_k_plus_1 - t_k) * (xi + 1) / 2

    # Append to the list of all tau points
    tau_all.extend(tau_j)

# Convert to a numpy array
tau_all = np.array(tau_all)

print("xi",xi)
print(len(numpy.unique(tau_all)) < len(tau_all))


yy_ref = numpy.zeros((16, len(tau_all)))
uu_ref = numpy.zeros((2, len(tau_all)))
...
for i in range(len(tau_all)):
    yy_ref[:, i] = state_vector_function(tau_all[i])
    # print("YY_REF",yy_ref[:, i])
    # print("shape of yy_ref",yy_ref.shape)
    uu_ref[:, i] = control_vector_function(tau_all[i])

print("YY_REF",yy_ref.shape)
print("UU_REF",uu_ref.shape)
print("state, yy values",yy_ref)
...
w.append(Xk)
lbw.append(np.zeros(state_size).flatten())  # Ensure 1D array
ubw.append(np.zeros(state_size).flatten())  # Ensure 1D array
w0.append(np.zeros(state_size).flatten())  # Ensure 1D array
x_plot.append(Xk)
...
for k in range(N_intervals):
    ...
    # w.append(Uk)
    # lbw.append(np.array([-1, -1]).flatten())  # Ensure 1D array
    # ubw.append(np.array([1, 1]).flatten())  # Ensure 1D array
    # w0.append(U_guess[:, k])  # Ensure 1D array
    # print("U_guess",U_guess[:, k])
    # u_plot.append(Uk)
    ...
    if k == 0:
        w.append(tk)
        lbw.append([0 if k == 0 else -np.inf])  # Ensure 1D array
        ubw.append([0 if k == 0 else -np.inf])  # Ensure 1D array
        w0.append([0])  # Ensure 1D array
    # elif k == N_intervals - 1:
    #     w0.append([T_total])
    # else:
    #     w0.append([np.inf])
    ...
    for j in range(N_collocation):
        ...
        Xc.append(Xkj)
        w.append(Xkj)
        lbw.append(np.full(state_size, -np.inf).flatten())  # Ensure 1D array
        ubw.append(np.full(state_size, np.inf).flatten())  # Ensure 1D array
        print("index k, j, total",k,j,k * N_collocation + j)
        w0.append(yy_ref[:,k * N_collocation + j].flatten())  # Ensure 1D array

        # Controls
        Ukj = ca.MX.sym(f'U_{k}_{j}', control_size)
        w.append(Ukj)
        lbw.append(np.full(control_size, -1e-5).flatten())  # Lower bounds
        ubw.append(np.full(control_size, 1e-5).flatten())   # Upper bounds
        w0.append(U_guess[:,k * N_collocation + j].flatten())  # Initial guess

    # Enforce initial conditions for the state at the beginning of the interval (if needed)
    if k == 0:
        g.append(Xk - X_guess[:, 0])
        lbg.append(np.zeros(state_size).flatten())  # Ensure 1D array
        ubg.append(np.zeros(state_size).flatten())  # Ensure 1D array

    # Loop over collocation points
    Xk_end = ca.mtimes(C_ext[0:state_size, 0:state_size], Xk)
    for j in range(1, N_collocation):
        ...
        dx_dt = Dynamics_casadi(tk, Xc[j - 1], param, Ukj)
        ...
        cost_jk = con_chief_deputy_angle_casadi(ca.mtimes(T_inv, Xc[j-1]), param)**2

        J = 0 #J - Xc[j-1][6] # B[j] * cost_jk * dt
    ...
    lbw.append(np.full(state_size, -np.inf).flatten())  # Ensure 1D array
    ubw.append(np.full(state_size, np.inf).flatten())  # Ensure 1D array
    w0.append(yy_ref[:,k * N_collocation].flatten())  # Ensure 1D array
    x_plot.append(Xk)
    ...
    # # Add inequaliy constrain for yaw rates
    # g.append(ca.mtimes(T_inv, Xk)[14] - param["PHI_DOT"][0])
    # lbg.append([0])  # Ensure 1D array
    # ubg.append([np.inf])  # Ensure 1D array

    mtimes_result = ca.mtimes(T_inv, Xk)  # Compute T_inv * X_k
    g.append(mtimes_result[14])  # T_inv * X_k[14] > T1
    lbg.append(np.full(1,param["PHI_DOT"][0]).flatten())  # Ensure 1D array
    ubg.append(np.full(1,param["PHI_DOT"][1]).flatten())  # Ensure 1D array

    g.append(mtimes_result[15])  # T_inv * X_k[15] > T2
    lbg.append(np.full(1,param["PHI_DOT"][0]).flatten())  # Ensure 1D array
    ubg.append(np.full(1,param["PHI_DOT"][1]).flatten())  # Ensure 1D array
...
w0 = np.concatenate(w0)  # Ensure all arrays are 1D
lbw = np.concatenate(lbw)  # Ensure all arrays are 1D
ubw = np.concatenate(ubw)  # Ensure all arrays are 1D
lbg = np.concatenate(lbg)  # Ensure all arrays are 1D
ubg = np.concatenate(ubg)  # Ensure all arrays are 1D

print("Initial guess (w0):", w0)
print("NaN in w0:", np.isnan(w0).any())
print("Inf in w0:", np.isinf(w0).any())

g_func = ca.Function('g_func', [w], [g])
g_val = g_func(w0)
print("Equality constraints (g):", g_val)
print("NaN in g:", np.isnan(g_val).any())
print("Inf in g:", np.isinf(g_val).any())


jac_g = ca.jacobian(g, w)
jac_g_func = ca.Function('jac_g_func', [w], [jac_g])
jac_g_val = jac_g_func(w0)
print("Jacobian of equality constraints (jac_g_x):", jac_g_val)
print("NaN in jac_g_x:", np.isnan(jac_g_val).any())
print("Inf in jac_g_x:", np.isinf(jac_g_val).any())

# Check specific entry in Jacobian
row, col = 704, 919
print(f"Jacobian entry at row {row}, col {col}:", jac_g_val[row, col])
...
solver_options = {
    ...
    # 'ipopt.max_iter': 10,     # Limit iterations for debugging
    'ipopt.tol': 1e-6,  # Increase tolerance
    'ipopt.mu_init': 1e-3,  # Reduce initial barrier parameter
    ...
    'ipopt.nlp_scaling_method': 'gradient-based',  # Enable automatic scaling
}

# show the size of the decision variables
print("Size of the decision variables",w.shape)
print("Size of the constraints",g.shape)
print("Size of the lower bounds",lbw.shape)
print("Size of the upper bounds",ubw.shape)
print("Size of the initial guess",w0.shape)
print("Size of the lower bounds for constraints",lbg.shape)
print("Size of the upper bounds for constraints",ubg.shape)

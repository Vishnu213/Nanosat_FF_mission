...
for k in range(N_intervals):
    ...
    if k == 0:
        ...
        ubw.append([T_total if k == N_intervals - 1 else np.inf])  # Ensure 1D array
    ...
    for j in range(N_collocation):
        ...
        w.append(Ukj)
        lbw.append(np.full(control_size, -1).flatten())  # Lower bounds
        ubw.append(np.full(control_size, 1).flatten())   # Upper bounds
    ...
    for j in range(1, N_collocation):
        ...
        cost_jk = con_chief_deputy_angle_casadi(Xc[j - 1], param)
        J = J + B[j] * cost_jk * dt
